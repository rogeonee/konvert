...
const Avif = () => {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      const validFiles = filterAvifFiles(acceptedFiles);

      if (acceptedFiles.length !== validFiles.length) {
        toast({
          title: 'Non-AVIF files were skipped.',
          description: 'It is a AVIF converter after all.',
  ...
  const handleAddMore = () => {
    ...
    input.onchange = () => {
      ...
      const validFiles = filterAvifFiles(files);
      if (files.length !== validFiles.length) {
        toast({
          title: 'Non-AVIF files were skipped.',
          description: 'Only AVIF files are supported.',
          //className: 'bg-white',
        });
      }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        <div className="flex min-h-[calc(100vh-8rem)] sm:min-h-[calc(100vh-6rem)] flex-col gap-4 lg:gap-6 lg:p-6">
          ...
          >
            <input {...getInputProps()} id="fileInput" accept=".avif" />
            {fields.length === 0 ? (
              // Empty dropzone
              <div className="flex flex-1 items-center justify-center p-2 sm:p-4 h-empty-base sm:h-empty-base-sm md:h-empty-base-md lg:h-empty-base-lg">
                <div className="flex flex-col items-center gap-1 text-center">
                  <h2 className="text-xl sm:text-2xl font-bold tracking-tight">
                    {isDragActive
                      ? 'Drop the files here'
                      : 'Drop AVIF files or pick manually'}
                  </h2>
